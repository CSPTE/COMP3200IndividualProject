...
public class SubtaskComponent extends ConstraintLayout {
    ...
    private void updatePointcardFile(){
        ...
        String days = getTaskData(homeSubtaskbarContext.getFilesDir(), parentName);
        ArrayList<String> daysArray = daysToDaysArray(days); //Days To Array
        ArrayList<String> taskDates = getLogFile(homeSubtaskbarContext.getFilesDir(), parentName);
        ...
        //Task Points
        int oldPoints = getcurrentPoints(homeSubtaskbarContext.getFilesDir(), parentName);
        int newPoints = oldPoints + (Math.round(1 * streakInt * (percentage/100)));
        updatePointFile(homeSubtaskbarContext.getFilesDir(), parentName, newPoints);

        //AllTime Score
        int oldPointsAllTime = getcurrentPointsAllTime(homeSubtaskbarContext.getFilesDir());
        int newPointsAllTime = oldPointsAllTime + (Math.round(1 * streakInt * (percentage/100)));
        updateAllTimePointFile(homeSubtaskbarContext.getFilesDir(), newPointsAllTime);
        ...
        boolean resetMonth = checkIfMonthNeedsResetting(homeSubtaskbarContext.getFilesDir(), currentDateMonth);
        if (resetMonth){
            int oldPointsMonth = 0;
            int newPointsMonth = oldPointsMonth + (Math.round(1 * streakInt * (percentage/100)));
            updateMonthPointFileDated(homeSubtaskbarContext.getFilesDir(), newPointsMonth, currentDateMonth);
        } else {
            int oldPointsMonth = getcurrentPointsMonth(homeSubtaskbarContext.getFilesDir());
            int newPointsMonth = oldPointsMonth + (Math.round(1 * streakInt * (percentage/100)));
            updateMonthPointFile(homeSubtaskbarContext.getFilesDir(), newPointsMonth, currentDateMonth);
        }
        ...
        if(currentDateWeeklyDay.equals("Sunday")){
            ...
            if(resetWeek){
                ...
                updateWeekPointFileDated(homeSubtaskbarContext.getFilesDir(), newPointsWeek, currentDateWeekly);
            } else {
